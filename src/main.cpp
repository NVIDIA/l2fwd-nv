...
static int l2fwd_workload 				= CPU_WORKLOAD;
static int l2fwd_port_id 				= 0;
static uint32_t l2fwd_data_room_size	= DEFAULT_MBUF_DATAROOM;
static uint32_t l2fwd_pkt_burst_size 	= MAX_MBUFS_BURST;
static int l2fwd_num_pipelines 			= 1;
...
static uint64_t l2fwd_pktime_ns			= 0;
static int l2fwd_buffer_split 			= 0;
...
static struct rte_eth_conf port_conf = {
	.rxmode = {
				...
				.max_rx_pkt_len = l2fwd_data_room_size,
				.split_hdr_size = 0,
				.offloads = 0,
			},
	.txmode = {
...
static void print_stats(void)
{
	...
	printf("\tDPDK RX\n");
	for (index_queue = 0; index_queue < l2fwd_num_pipelines; index_queue++)
		printf
...
void print_opts(void)
{
	...
	printf("Pipelines = %d (%d RX cores and %d TX cores)\n", l2fwd_num_pipelines, l2fwd_num_pipelines, l2fwd_num_pipelines);
	...
	else
	{
		...
		if (l2fwd_workload == GPU_GRAPHS_WORKLOAD)
			printf("Workload type = GPU_GRAPHS_WORKLOAD\n");
		if (workload_with_gpu(l2fwd_workload) || l2fwd_mem_type == MEM_DEVMEM)
		{
			CUDA_CHECK(cudaSetDevice(l2fwd_gpu_device_id));
...
static int parse_args(int argc, char **argv)
{
	...
	while ((opt =
		...
		switch (opt) {
			case 'b':
				...
				if (l2fwd_pkt_burst_size < 0 || l2fwd_pkt_burst_size > MAX_MBUFS_BURST) {
			...
			case 'd':
				l2fwd_data_room_size = l2fwd_parse_dataroom_size(optarg);
			...
			case 'p':
				l2fwd_num_pipelines = l2fwd_parse_pipelines(optarg);
				if (l2fwd_num_pipelines <= 0) {
			...
			case 't':
				l2fwd_pktime_ns = l2fwd_parse_wtime(optarg);
				if ((l2fwd_pktime_ns < 0) || (l2fwd_pktime_ns > MAX_WTIME_NS)) {
					fprintf(stderr, "Workload packet time should be between 0 and %d ns (%lu provided)\n", MAX_WTIME_NS, l2fwd_pktime_ns);
					l2fwdnv_usage(prgname);
					return -1;
				}
			...
			case 'z':
				l2fwd_warmup_packets = l2fwd_parse_warmup_packets(optarg);
	...
	if (l2fwd_mem_type == MEM_DEVMEM || workload_with_gpu(l2fwd_workload)) {
	...
	if ( ((l2fwd_num_pipelines * 2) + 1) > (int)rte_lcore_count()) {
		fprintf(stderr,
			"Required l2fwd_num_pipelines+1 (%d), cores launched=(%d)\n",
			l2fwd_num_pipelines + 1, rte_lcore_count());
		return -1;
	}

	if (l2fwd_num_pipelines > MAX_RX_QUEUE_PER_LCORE) {
		fprintf(stderr, "RX queues %d > MAX_RX_QUEUE_PER_LCORE %d\n", l2fwd_num_pipelines, MAX_RX_QUEUE_PER_LCORE);
		return -1;
	}
	...
	if(l2fwd_buffer_split && l2fwd_workload == CPU_WORKLOAD) {
...
static int rx_core(void *arg)
{
	...
	while (ACCESS_ONCE(force_quit) == 0 && ACCESS_ONCE(p_v->pipeline_force_quit) == 0)
	{
		...
		while (
				...
				nb_rx < (l2fwd_pkt_burst_size - GAP_PKTS)
		)
		{
			nb_rx += rte_eth_rx_burst(l2fwd_port_id, pipeline_idx, 
										&(blist[bindex].mbufs[nb_rx]),
										(l2fwd_pkt_burst_size - nb_rx));
		}
		...
		{
			if(l2fwd_performance_packets > 0 && p_v->rx_pkts >= l2fwd_warmup_packets)
			{
				p_v->rx_pkts = 0;
		...
		if (p_v->workload_type == GPU_PK_WORKLOAD || p_v->workload_type == GPU_GRAPHS_WORKLOAD) {
			...
			workload_launch_gpu_processing(
								blist[bindex].addr, blist[bindex].num_mbufs, &(blist[bindex].status), l2fwd_pktime_ns,
								MAC_CUDA_BLOCKS, MAC_THREADS_BLOCK, p_v->c_stream
							);
			POP_RANGE;
		}

		if(p_v->start_rx_measure == true && l2fwd_performance_packets > 0 && p_v->rx_pkts >= l2fwd_performance_packets)
		{
...
static int tx_core(void *arg)
{
	...
	while(ACCESS_ONCE(force_quit) == 0 && ACCESS_ONCE(p_v->pipeline_force_quit) == 0)
	{
		...
		{
			if(l2fwd_performance_packets > 0 && p_v->tx_pkts >= l2fwd_warmup_packets)
			{
				p_v->tx_pkts = 0;
		...
		if (p_v->workload_type == CPU_WORKLOAD) {
			...
			workload_macswap_cpu(blist[bindex].addr, blist[bindex].num_mbufs, l2fwd_pktime_ns);
			POP_RANGE;
		}
		...
		while(
			...
			nb_tx += rte_eth_tx_burst(l2fwd_port_id, pipeline_idx,
		...
		if(p_v->start_tx_measure == true && l2fwd_performance_packets > 0 && p_v->tx_pkts >= l2fwd_performance_packets)
		{
...
int main(int argc, char **argv)
{
	...
	if(l2fwd_buffer_split)
	{
		mpool_header = rte_pktmbuf_pool_create("sysmem_mpool_hdr", l2fwd_nb_mbufs,
												l2fwd_mempool_cache, 0,
	...
	//// PORT 0 SETUP
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	printf("Initializing port %u with %d RX queues and %d TX queues...\n", l2fwd_port_id, l2fwd_num_pipelines, l2fwd_num_pipelines);
	ret = rte_eth_dev_configure(l2fwd_port_id, l2fwd_num_pipelines, l2fwd_num_pipelines, &port_conf);
	if (ret < 0)
	...
	print_rx_offloads(port_conf.rxmode.offloads);
	printf("\n");
	printf("Port TX offloads: ");
	print_tx_offloads(port_conf.txmode.offloads);
	printf("\n");
	...
	for (index_queue = 0; index_queue < l2fwd_num_pipelines; index_queue++) {
		...
		if(l2fwd_buffer_split)
			ret = rte_eth_rx_queue_setup(l2fwd_port_id, index_queue, nb_rxd, socketid, &rxconf_qsplit, NULL);
		else
			ret = rte_eth_rx_queue_setup(l2fwd_port_id, index_queue, nb_rxd, socketid, NULL, mpool_payload);
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	pipeline_v.reserve(l2fwd_num_pipelines);
	for(int index_p = 0; index_p < l2fwd_num_pipelines; index_p++)
		pipeline_v[index_p] = new Pipeline(index_p, l2fwd_workload, l2fwd_pktime_ns, &gdr_descr, index_p, index_p);
	...
	//// START RX/TX CORES
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	for(icore = 0; icore < l2fwd_num_pipelines; icore++)
	{
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	printf("Closing port %d...", l2fwd_port_id);
	rte_eth_dev_stop(l2fwd_port_id);
	rte_eth_dev_close(l2fwd_port_id);
